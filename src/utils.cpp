...
{
    cv::Mat mat(m->nrows, m->ncols, cv::DataType<float>::type);
    for (int i = 0; i < m->nrows; i++)
        for (int j = 0; j < m->ncols; j++)
        {
            mat.at<float>(i, j) = matd_get(m, i, j);
        }
    return mat;
}
...
{
    cv::Mat m(3, v.size(), cv::DataType<float>::type);
...
{
    cv::Mat m(2, v.size(), cv::DataType<float>::type);
    for (int i = 0; i < v.size(); i++)
    {
        m.at<float>(0, i) = v[i].x;
        m.at<float>(1, i) = v[i].y;
...
std::vector<cv::Point3f> defineCubeWithPoints(float size)
{
    ...
    t.at<float>(0, 0) = -size / 2;
    t.at<float>(1, 0) = -size / 2;
    t.at<float>(2, 0) = 0;
...
void rotatePoints(std::vector<cv::Point3f> &cube, cv::Mat R)
{
    for (auto &point : cube)
    {
        ...
        float tempy = point.y;
        float tempz = point.z;
        point.x = R.at<float>(0, 0) * tempx + R.at<float>(0, 1) * tempy +
                  R.at<float>(0, 2) * tempz;
        point.y = R.at<float>(1, 0) * tempx + R.at<float>(1, 1) * tempy +
                  R.at<float>(1, 2) * tempz;
        point.z = R.at<float>(2, 0) * tempx + R.at<float>(2, 1) * tempy +
                  R.at<float>(2, 2) * tempz;
    }
}
...
void translatePoints(std::vector<cv::Point3f> &cube, cv::Mat t)
{
    for (auto &point : cube)
    {
        point.x += t.at<float>(0, 0);
        point.y += t.at<float>(1, 0);
        point.z += t.at<float>(2, 0);
    }
}
              ...
              cv::Scalar &color)
{
    ...
    cv::Mat transformationMatrix = cv::Mat::eye(4, 4, cv::DataType<float>::type);
...
std::vector<cv::Point3f> defineAxesWithPoints(float size)
{
...
cv::Mat fromRPY(float roll, float pitch, float yaw)
{
    ...
    cv::Mat R_y = (cv::Mat_<float>(3, 3) << cos(pitch), 0, sin(pitch),
                   0, 1, 0,
                   -sin(pitch), 0, cos(pitch));
    // Calculate rotation about z axis
    cv::Mat R_z = (cv::Mat_<float>(3, 3) << cos(yaw), -sin(yaw), 0,
                   sin(yaw), cos(yaw), 0,
                   0, 0, 1);
...
cv::Mat getRotationMatrix(float roll, float pitch, float yaw)
{
    ...
    float c_roll = std::cos(roll);
    float s_roll = std::sin(roll);
    float c_pitch = std::cos(pitch);
    float s_pitch = std::sin(pitch);
    float c_yaw = std::cos(yaw);
    float s_yaw = std::sin(yaw);

    R.at<float>(0, 0) = c_yaw * c_pitch;
    R.at<float>(0, 1) = c_yaw * s_pitch * s_roll - s_yaw * c_roll;
    R.at<float>(0, 2) = c_yaw * s_pitch * c_roll + s_yaw * s_roll;

    R.at<float>(1, 0) = s_yaw * c_pitch;
    R.at<float>(1, 1) = s_yaw * s_pitch * s_roll + c_yaw * c_roll;
    R.at<float>(1, 2) = s_yaw * s_pitch * c_roll - c_yaw * s_roll;

    R.at<float>(2, 0) = -s_pitch;
    R.at<float>(2, 1) = c_pitch * s_roll;
    R.at<float>(2, 2) = c_pitch * c_roll;
...
{
    cv::Mat R = cv::Mat::zeros(3, 3, CV_64F);
    R.at<float>(0, 0) = tf.T.at<float>(0, 0);
    R.at<float>(0, 1) = tf.T.at<float>(0, 1);
    R.at<float>(0, 2) = tf.T.at<float>(0, 2);
    R.at<float>(1, 0) = tf.T.at<float>(1, 0);
    R.at<float>(1, 1) = tf.T.at<float>(1, 1);
    R.at<float>(1, 2) = tf.T.at<float>(1, 2);
    R.at<float>(2, 0) = tf.T.at<float>(2, 0);
    R.at<float>(2, 1) = tf.T.at<float>(2, 1);
    R.at<float>(2, 2) = tf.T.at<float>(2, 2);
    return R;
}
...
cv::Mat getTranslationFromTransform(Transform &tf)
{
    ...
    t.at<float>(0, 0) = tf.T.at<float>(0, 3);
    t.at<float>(1, 0) = tf.T.at<float>(1, 3);
    t.at<float>(2, 0) = tf.T.at<float>(2, 3);
    return t;
}
...
cv::Mat truncateVector(cv::Mat t, float fraction)
{
    ...
    for (int i = 0; i < shorter.rows; i++)
        for (int j = 0; j < shorter.cols; j++)
            shorter.at<float>(i, j) = fraction * shorter.at<float>(i, j);
    return shorter;
}
...
float getAngularDifference(float th1, float th2)
{
    ...
    float diff = atan2((cos(th1) * sin(th2) - cos(th2) * sin(th1)),
